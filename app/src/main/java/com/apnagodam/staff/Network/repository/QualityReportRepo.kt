...
class QualityReportRepo @Inject constructor(val apiService: ApiService):BaseApiResponse() {
    ...
    suspend fun getQualityReportList(limit:String,page:String,inOut:String,search:String): Flow<NetworkResult<FirstQuilityReportListResponse>> {
        return  flow {emit(safeApiCall { apiService.getf_qualityReportsList(limit,page,inOut,search) })  }.flowOn(Dispatchers.IO)
    }
...
//        return  flow {emit(safeApiCall { apiService.uploadFirstQualityReports(uploadFirstQualityPostData) })  }.flowOn(Dispatchers.IO)
//    }
    suspend fun getSQualityReportList(limit:String,page:String,inOut:String,search:String): Flow<NetworkResult<SecoundQuilityReportListResponse>> {
        return  flow {emit(safeApiCall { apiService.getS_qualityReportsList(limit,page,inOut,search) })  }.flowOn(Dispatchers.IO)
    }
    ...
    suspend fun uploadSecondQualityReport(uploadSecoundQualityPostData:UploadSecoundQualityPostData): Flow<NetworkResult<LoginResponse>> {
        return  flow {emit(safeApiCall { apiService.uploadSecoundQualityReports(uploadSecoundQualityPostData) })  }.flowOn(Dispatchers.IO)
    }
}
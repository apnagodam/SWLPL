...
class DispleaseViewModel @Inject constructor(
    ...
    val displeasedRejectResponse = MutableLiveData<NetworkResult<BaseResponse>>()
    ...
    fun rejectDispleasedRequest(id: String, notes: String) = viewModelScope.launch {
        repo.rejectDispleasedRequest(id, notes).collect {
            displeasedRejectResponse.value = it
        }
    }
}
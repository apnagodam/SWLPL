...
@HiltViewModel
class QualitReportViewModel @Inject constructor(val qualityReportRepo: QualityReportRepo,application: Application):AndroidViewModel(application) {
    ...
    fun getFirstQualityListing(limit:String,page:String,inOut:String,search:String) = viewModelScope.launch {
        qualityReportRepo.getQualityReportList(limit,page,inOut,search).collect(){
            fQualityResponse.value = it
        }
    }

    fun getSecondQualityListing(limit:String,page:String,inOut:String,search:String) = viewModelScope.launch {
        qualityReportRepo.getSQualityReportList(limit,page,inOut,search).collect(){
            sQualityResponse.value = it
        }
    }
    fun uploadSecondQualityReport(uploadSecoundQualityPostData: UploadSecoundQualityPostData) = viewModelScope.launch {
        qualityReportRepo.uploadSecondQualityReport(uploadSecoundQualityPostData).collect(){

...
import com.apnagodam.staff.activity.feedbackemp.ApprovalRequestFeedbackListClass;
...
public class ApprovalRequestFeedbackListAdpter extends RecyclerView.Adapter<ApprovalRequestFeedbackListAdpter.CommudityResponseViewHolder> {
    private List<FeedbackApprovalLisPojo.Datum> commudityResponseList;
    private Context context;
    private BaseActivity activity;

    public ApprovalRequestFeedbackListAdpter(List<FeedbackApprovalLisPojo.Datum> body, ApprovalRequestFeedbackListClass approvalRequestFeedbackListClass, BaseActivity activity) {
        this.commudityResponseList = body;
        this.context = approvalRequestFeedbackListClass;
    ...
    public void onBindViewHolder(CommudityResponseViewHolder holder, final int position) {
        ...
        if (commudityResponseList.get(position).getFirstName() != null) {
            holder.tv_gatepass.setText( (commudityResponseList.get(position).getFirstName() + " " + commudityResponseList.get(position).getLastName() + "(" + commudityResponseList.get(position).getEmpId() + ")"));
        } else {
            holder.tv_gatepass.setText("-");
        }
        if (commudityResponseList.get(position).getDesignation() != null) {
            holder.tv_name.setText(" " + commudityResponseList.get(position).getDesignation());
        } else {
        ...
        if (commudityResponseList.get(position).getUpdatedAt() != null) {
            ...
            try{
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ...
                    Date date = null;
                    try {
                        date = inputFormatter.parse((commudityResponseList.get(position).getUpdatedAt()));
        ...
        holder.tv_action.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (context instanceof ApprovalRequestFeedbackListClass) {
                    ((ApprovalRequestFeedbackListClass) context).cancelConv(position);
                }
            }
        });
        holder.tv_action_apprved.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (context instanceof ApprovalRequestFeedbackListClass) {
                    ((ApprovalRequestFeedbackListClass) context).ApprovedConv(position);
                }
            }
        });
        holder.view1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (context instanceof ApprovalRequestFeedbackListClass) {
                    ((ApprovalRequestFeedbackListClass) context).ViewData(position);
                }
            }
        });
    }

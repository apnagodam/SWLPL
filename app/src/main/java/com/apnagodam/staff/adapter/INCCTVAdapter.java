...
public class INCCTVAdapter extends BaseRecyclerViewAdapter {
    private List<CCTVListPojo.Datum> Leads;
    private Context context;
    private BaseActivity activity;
    public INCCTVAdapter(List<CCTVListPojo.Datum> leads, INCCTVListingActivity outFirstkanthaParchiListingActivity, BaseActivity activity) {
        this.Leads = leads;
           ...
           /* if (position==0){
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof INCCTVListingActivity) {
                        ((INCCTVListingActivity) context).   ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof INCCTVListingActivity) {
                        ((INCCTVListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                            ...
                            for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("33")) {
                                    if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                        if (Leads.get(position).getS_q_case_id_in()!=null){

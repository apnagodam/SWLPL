...
public class OutFirstkanthaparchiAdapter extends BaseRecyclerViewAdapter {
    private List<FirstkanthaParchiListResponse.Datum> Leads;
    private Context context;
    private BaseActivity activity;
    public OutFirstkanthaparchiAdapter(List<FirstkanthaParchiListResponse.Datum> leads, OutFirstkanthaParchiListingActivity outFirstkanthaParchiListingActivity, BaseActivity activity) {
        this.Leads = leads;
           ...
           /* if (position==0){
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutFirstkanthaParchiListingActivity) {
                        ((OutFirstkanthaParchiListingActivity) context).   ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutFirstkanthaParchiListingActivity) {
                        ((OutFirstkanthaParchiListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
         ...
         for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
             if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("20")) {
                 if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                     if (Leads.get(position).f_q_case_id !=null){

...
public class OutSecoundkanthaparchiAdapter extends BaseRecyclerViewAdapter {
    private List<SecoundkanthaParchiListResponse.Datum> Leads;
    private Context context;
    private BaseActivity activity;
    public OutSecoundkanthaparchiAdapter(List<SecoundkanthaParchiListResponse.Datum> leads, OutSecoundkanthaParchiListingActivity
            outSecoundkanthaParchiListingActivity, BaseActivity activity) {
        this.Leads = leads;
        this.context = outSecoundkanthaParchiListingActivity;
           ...
           /* if (position==0){
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutSecoundkanthaParchiListingActivity) {
                        ((OutSecoundkanthaParchiListingActivity) context).   ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutSecoundkanthaParchiListingActivity) {
                        ((OutSecoundkanthaParchiListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                            ...
                            for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("20")) {
                                    if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                        if (Leads.get(position).getS_q_case_id()!=null){

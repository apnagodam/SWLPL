...
public class OUTTruckBookAdapter extends BaseRecyclerViewAdapter {

    private List<AllTruckBookListResponse.Datum> Leads;
    private Context context;
    private BaseActivity activity;

    public OUTTruckBookAdapter(List<AllTruckBookListResponse.Datum> leads, OUTTruckBookListingActivity
           ...
           /* if (position==0){
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTTruckBookListingActivity) {
                        ((OUTTruckBookListingActivity) context).ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTTruckBookListingActivity) {
                        ((OUTTruckBookListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                          ...
                          /*  new Thread() {
                                public void run() {*/
                                    for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                        if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("15")) {
                                            if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                                if (Leads.get(position).getDelivery_order_case_id()!=null){

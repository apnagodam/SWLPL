...
public class OutFirstQualityReportAdapter extends BaseRecyclerViewAdapter {
    private List<FirstQuilityReportListResponse.Datum> Leads;
    private Context context;
    private BaseActivity activity;
    public OutFirstQualityReportAdapter(List<FirstQuilityReportListResponse.Datum> leads, OutFirstQualityReportListingActivity outFirstQualityReportListingActivity, BaseActivity activity) {
        this.Leads = leads;
        this.context = outFirstQualityReportListingActivity;
           ...
           /* if (position==0){
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutFirstQualityReportListingActivity) {
                        ((OutFirstQualityReportListingActivity) context).ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OutFirstQualityReportListingActivity) {
                        ((OutFirstQualityReportListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                            ...
                            for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("28")) {
                                    if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                        if (Leads.get(position).getLabour_book_case_id()!=null){

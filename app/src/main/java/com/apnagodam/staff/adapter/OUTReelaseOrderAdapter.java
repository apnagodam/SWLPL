...
public class OUTReelaseOrderAdapter extends BaseRecyclerViewAdapter {

    private List<ReleaseOrderPojo.Datum> Leads;
    private Context context;
    private BaseActivity activity;

    public OUTReelaseOrderAdapter(List<ReleaseOrderPojo.Datum> leads, OUTRelaseOrderListingActivity
            outRelaseOrderListingActivity, BaseActivity activity) {
        this.Leads = leads;
        this.context = outRelaseOrderListingActivity;
    ...
    class DefaultersTopHolder extends BaseViewHolder<PricingDataBinding> {
        ...
        public void onBind(int position) {
           ...
           /* if (position==0){
            ...
            if (Leads.get(position).getROCaseId()!=null){
                binding.tvAction.setVisibility(View.GONE);
                binding.tvActionDone.setVisibility(View.GONE);
                ...
                binding.tvAction.setVisibility(View.GONE);
                binding.tvPhone.setText(context.getResources().getString(R.string.upload__relase));
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTRelaseOrderListingActivity) {
                        ((OUTRelaseOrderListingActivity) context).ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTRelaseOrderListingActivity) {
                        ((OUTRelaseOrderListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                                ...
                                public void run() {*/
                                    for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                        if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("24")) {
                                            if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                                if (Leads.get(position).getPricing_caseId()!=null){

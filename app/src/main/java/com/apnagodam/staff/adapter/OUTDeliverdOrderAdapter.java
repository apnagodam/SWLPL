...
public class OUTDeliverdOrderAdapter extends BaseRecyclerViewAdapter {

    private List<ReleaseOrderPojo.Datum> Leads;
    private Context context;
    private BaseActivity activity;

    public OUTDeliverdOrderAdapter(List<ReleaseOrderPojo.Datum> leads, OUTDeliverdOrderListingActivity
            outDeliverdOrderListingActivity, BaseActivity activity) {
        this.Leads = leads;
        this.context = outDeliverdOrderListingActivity;
    ...
    class DefaultersTopHolder extends BaseViewHolder<PricingDataBinding> {
        ...
        public void onBind(int position) {
           ...
           /* if (position==0){
            ...
            if (Leads.get(position).getdOCaseId()!=null){
                binding.tvAction.setVisibility(View.GONE);
                binding.tvActionDone.setVisibility(View.GONE);
            ...
            binding.view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTDeliverdOrderListingActivity) {
                        ((OUTDeliverdOrderListingActivity) context).ViewData(position);
                    }
                }
            });
            binding.tvPhone.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (context instanceof OUTDeliverdOrderListingActivity) {
                        ((OUTDeliverdOrderListingActivity) context).checkVeehicleNo(position);
                    }
                }
            });
        }
                                ...
                                public void run() {*/
                                    for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                                        if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("25")) {
                                            if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                                                //boolean name = SharedPreferencesRepository.getDataManagerInstance().isReleaseSave();
                                               // if (name) {
                                              if (Leads.get(position).getRelese_case_id()!=null){
                                                    tvPhone.setText(context.getResources().getString(R.string.upload__delierverd));
                                                    tvPhone.setBackgroundColor(context.getResources().getColor(R.color.yellow));

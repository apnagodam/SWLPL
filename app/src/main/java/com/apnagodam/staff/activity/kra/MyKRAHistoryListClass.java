...
import com.apnagodam.staff.databinding.KraListBinding;
...
public class MyKRAHistoryListClass extends BaseActivity<KraListBinding> {
    private KRAHistoryListAdpter kraHistoryListAdpter;
    ...
    protected void setUp() {
        ...
        binding.filterIcon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlertDialog.Builder builder = new AlertDialog.Builder(MyKRAHistoryListClass.this);
                LayoutInflater inflater = ((Activity) MyKRAHistoryListClass.this).getLayoutInflater();
            ...
            for (int i = 0; i < SharedPreferencesRepository.getDataManagerInstance().getUserPermission().size(); i++) {
                if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getPermissionId().equalsIgnoreCase("41")) {
                    if (SharedPreferencesRepository.getDataManagerInstance().getUserPermission().get(i).getEdit() == 1) {
                        startActivity(UploadMyKRAClass.class);
                    }
                }
            }
        }
        ...
        apiService.getConvancyList("15", pageOffset, SerachKey).enqueue(new NetworkCallback<AllConvancyList>(getActivity()) {
            ...
            protected void onSuccess(AllConvancyList body) {
                ...
                if (body.getData().getDataa().size() == 0) {
                    ...
                    kraHistoryListAdpter.notifyDataSetChanged();
                } else {
                    binding.txtemptyMsg.setVisibility(View.GONE);
                    binding.fieldStockList.setVisibility(View.VISIBLE);
    ...
    private void setDataAdapter() {
        binding.fieldStockList.addItemDecoration(new DividerItemDecoration(MyKRAHistoryListClass.this, LinearLayoutManager.HORIZONTAL));
        binding.fieldStockList.setHasFixedSize(true);
        binding.fieldStockList.setNestedScrollingEnabled(false);
        LinearLayoutManager horizontalLayoutManager = new LinearLayoutManager(MyKRAHistoryListClass.this, LinearLayoutManager.VERTICAL, false);
        binding.fieldStockList.setLayoutManager(horizontalLayoutManager);
        kraHistoryListAdpter = new KRAHistoryListAdpter(getOrdersList, MyKRAHistoryListClass.this, getActivity());
        binding.fieldStockList.setAdapter(kraHistoryListAdpter);
    }
    ...
    public void ViewData(int position) {
        ...
        final AlertDialog.Builder builder = new AlertDialog.Builder(MyKRAHistoryListClass.this, R.style.CustomAlertDialog);
        LayoutInflater inflater = MyKRAHistoryListClass.this.getLayoutInflater();

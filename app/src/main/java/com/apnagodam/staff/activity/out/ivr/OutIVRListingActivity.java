package com.apnagodam.staff.activity.out.ivr;
...
public class OutIVRListingActivity extends BaseActivity<ActivityListingBinding> {
    private OutIVRVAdapter outFirstkanthaparchiAdapter;
    ...
    protected void setUp() {
        ...
        binding.filterIcon.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                AlertDialog.Builder builder = new AlertDialog.Builder(OutIVRListingActivity.this);
                LayoutInflater inflater = ((Activity) OutIVRListingActivity.this).getLayoutInflater();
    ...
    private void setAdapter() {
        binding.rvDefaultersStatus.addItemDecoration(new DividerItemDecoration(OutIVRListingActivity.this, LinearLayoutManager.VERTICAL));
        binding.rvDefaultersStatus.setHasFixedSize(true);
        binding.rvDefaultersStatus.setNestedScrollingEnabled(false);
        LinearLayoutManager horizontalLayoutManager = new LinearLayoutManager(OutIVRListingActivity.this, LinearLayoutManager.VERTICAL, false);
        binding.rvDefaultersStatus.setLayoutManager(horizontalLayoutManager);
        outFirstkanthaparchiAdapter = new OutIVRVAdapter(AllCases, OutIVRListingActivity.this, getActivity());
        binding.rvDefaultersStatus.setAdapter(outFirstkanthaparchiAdapter);
    }

    private void getAllCases(String search) {
        showDialog();
        apiService.getIVRList("10", "" + pageOffset, "OUT",search).enqueue(new NetworkCallback<IVRListPojo>(getActivity()) {
            @Override
    ...
    public void ViewData(int position) {
        ...
        final AlertDialog.Builder builder = new AlertDialog.Builder(OutIVRListingActivity.this, R.style.CustomAlertDialog);
        LayoutInflater inflater = OutIVRListingActivity.this.getLayoutInflater();
    ...
    public void checkVeehicleNo(int postion) {
        ...
        startActivity(OutUploadIVRFileClass.class, bundle);
    }
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (weight.getText().toString().trim() == null || weight.getText().toString().trim().isEmpty()) {
                    ...
                    Utility.showDecisionDialog(OutIVRListingActivity.this, getString(R.string.alert), "Are you Sure? ", new Utility.AlertCallback() {
                        @Override
                        public void callback() {
                            String inventory_id = "" + AllCases.get(postion).getId();
                            Utility.showDecisionDialog(OutIVRListingActivity.this, getString(R.string.alert), "Are you Sure? ", new Utility.AlertCallback() {
                                @Override
                                public void callback() {
                                    ...
                                    apiService.doCreateEmpGatepasssUpdate(new EmpUpdateGartepassRequest(inventory_id,  weight.getText().toString().trim(), bags.getText().toString().trim())).enqueue(new NetworkCallback<LoginResponse>(getActivity()) {
                                        ...
                                        protected void onSuccess(LoginResponse body) {
                                            alertDialog.dismiss();
                                            Toast.makeText(OutIVRListingActivity.this, body.getMessage(), Toast.LENGTH_LONG).show();
                                            getAllCases("");
                                        }
                                    });
                                }

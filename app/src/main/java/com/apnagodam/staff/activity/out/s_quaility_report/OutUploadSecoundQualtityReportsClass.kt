...
//                                listOfQualityParams.add(Datum())
    ...
    private fun clickListner() {
        ...
        binding!!.ReportsImage.setOnClickListener { view ->
            PhotoFullPopupWindow(
                this, R.layout.popup_photo_full, view, reportFile, null
            )
        }
        binding!!.CommudityImage.setOnClickListener { view ->
            PhotoFullPopupWindow(
                this, R.layout.popup_photo_full, view, commudityFile, null
            )
        }
    }
    ...
    fun onNext() {
        ...
        for (i in listOfParams.indices) {
            var datum = UploadFirstQualityPostData.CommodityData(
                listOfParams[i].id,
                listOfParams[i].name,
                "",
                listOfParams[i].min,
        ...
        when (requestCode) {
            PermUtil.REQUEST_CODE_ASK_MULTIPLE_PERMISSIONS -> {
                ...
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(
                        this, "Approve permissions to open Pix ImagePicker", Toast.LENGTH_LONG
                    ).show()
                }
                return
            }
